...
第一阶段建议先单独观察 `/vio/odometry` 和 `/lio/odometry`，确认两者趋势一致后再使用 `--odom-source both`。

## 推荐落地顺序

1. 只启动 MID360 驱动，确认 `/livox/lidar` 和 `/livox/imu`。
2. 使用 `--pointcloud-source mid360`，确认 `/mid360/points` 和局部地图。
3. 使用 `--pointcloud-source both`，确认 `/perception/obstacle_points`。
4. 单独启用 FAST-LIO2，确认 `/lio/odometry` 稳定。
5. 最后使用 `--odom-source both`，让 EKF 同时融合 VIO 与 LIO 两路并列里程计输入。
