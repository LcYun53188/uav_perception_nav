...
| 位置 | 新文件 | 旧文件 | 用途 |
|------|--------|--------|------|
| oakd_imu_fusion/oakd_imu_fusion/ | `imu_fusion_node.py` | `oakd_imu_fusion_node.py` | 通用IMU融合节点 |
| oakd_imu_fusion/oakd_imu_fusion/ | `imu_tf_broadcaster.py` | `oakd_imu_tf_broadcaster.py` | 通用TF广播节点 |
| oakd_imu_fusion/launch/ | `imu_fusion.launch.py` | `oakd_imu_fusion.launch.py` | 多IMU支持的通用启动文件 |
| oakd_imu_fusion/launch/ | `oakd_imu_fusion.launch.py` | - | 向后兼容包装器 |
...
```bash
# 新推荐方式
ros2 launch oakd_imu_fusion imu_fusion.launch.py

# 或使用旧启动文件（向后兼容）
ros2 launch oakd_imu_fusion oakd_imu_fusion.launch.py
```

**默认主题和框架：**
- 原始IMU: `/imu/raw`（新名称，原为 `/oakd/imu/raw`可自定义）
- 融合IMU: `/imu`（新名称，原为 `/oakd/imu`可自定义）
- 框架ID: `imu_link`（新名称，原为 `oakd_imu_link`可自定义）

### 场景2：多IMU支持（新功能）

#### 配置2个IMU的完整示例

创建启动文件 `launch/multi_imu_example.launch.py`：
...
```bash
ros2 launch oakd_imu_fusion multi_imu_example.launch.py
```
...
启动文件已预留以下注释代码块，用于轻松添加更多IMU：

```python
# IMU 1 configuration (secondary) - 取消注释启用
# nodes.extend([...])

# IMU 2 configuration (tertiary) - 取消注释启用
# nodes.extend([...])
```
...
# 这两个命令等效
ros2 launch oakd_imu_fusion imu_fusion.launch.py
ros2 launch oakd_imu_fusion oakd_imu_fusion.launch.py  # 仍可用

# 这两个可执行程序等效
ros2 run oakd_imu_fusion imu_fusion_node
ros2 run oakd_imu_fusion oakd_imu_fusion_node  # 仍可用
```
...
<!-- 旧方式 -->
<launch>
  <include file="$(find oakd_imu_fusion)/launch/oakd_imu_fusion.launch.py"/>
</launch>

<!-- 新推荐 -->
<launch>
  <include file="$(find oakd_imu_fusion)/launch/imu_fusion.launch.py"/>
</launch>
```
